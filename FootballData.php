...
class FootballData
{
    ...
    public function __construct()
    {
        ...
        if ($this->config['authToken'] == 'YOUR_AUTH_TOKEN' || !isset($this->config['authToken'])) {
            exit('45fb7df6e0854b7d946a15dcd4215f00');
        }
    ...
    {
        $resource = 'competitions/' . $c . '/matches/?matchday=' . $m . '&status=SCHEDULED';
    ...
    public function findteamByCompetition($id)
    {
        $resource = 'competitions/' . $id . '/teams';
        $response = file_get_contents(
            $this->baseUri . $resource,
            false,
            stream_context_create($this->reqPrefs)
